...
class SODADSplit(object):
        ...
        while (left < imgW):
            ...
            while (up < imgH):
                ...
                if sign:
                    ...
                    save_path = osp.join(self.divImgDir, newFileName)
                    if self.ignFill and fillIgnImgPatch is not None and fillIgnImgPatch.size > 0:
                        cv2.imwrite(save_path, fillIgnImgPatch)
                    elif imgPatch is not None and imgPatch.size > 0:
                        cv2.imwrite(save_path, imgPatch)
                    else:
                        logger.warning(f"跳过保存空图像: {newFileName}")
                        continue  # 跳过这次循环，防止保存错误图像
    ...
    def _divAnn(self, annsWithPoly, patchBbox, imgId, imgPatch):
        ...
        # 修复后的安全过滤实现：
        # 如果 vldIndex 是布尔列表/数组：
        if isinstance(vldIndex, (list, np.ndarray)) and len(vldIndex) > 0 and isinstance(vldIndex[0], (bool, np.bool_)):
            bounds = [b for b, flag in zip(bounds, vldIndex) if flag and b != []]
        # 如果 vldIndex 是索引序列（list 或 ndarray）：
        else:
            bounds = [bounds[idx] for idx in vldIndex if bounds[idx] != []]
    ...
    def _getIgnMask(self, maskSz, catLst, bounds, left, up):
        ...
        objBboxAry = np.array(bboxLst)[~ignSign].astype(np.int32)
        ignBboxAry = np.array(bboxLst)[ignSign].astype(np.int32)
