...
"""
    ...
    参数：
    bboxes1: 边界框集合1，形状(B, m, 4)
    bboxes2: 边界框集合2，形状(B, n, 4)
    mode: 计算模式
    is_aligned: 是否对齐模式
    eps: 数值稳定性小量
    weight: Wasserstein距离参数（当前未使用）
...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6, weight=2):
    ...
    area1 = (bboxes1[..., 2] - bboxes1[..., 0]) * (
        bboxes1[..., 3] - bboxes1[..., 1]) # area1形状: (B, m)
    area2 = (bboxes2[..., 2] - bboxes2[..., 0]) * (
        bboxes2[..., 3] - bboxes2[..., 1]) # area2形状: (B, n)
    ...
    lt = torch.max(bboxes1[..., :, None, :2],
                    bboxes2[..., None, :, :2])  # [B, rows, cols, 2] 计算相交区域的左上角坐标
    rb = torch.min(bboxes1[..., :, None, 2:],
                    bboxes2[..., None, :, 2:])  # [B, rows, cols, 2] 计算相交区域的右下角坐标

    wh = (rb - lt).clamp(min=0)  # [B, rows, cols, 2] 计算相交区域的宽高（负数部分clamp为0;这里处理无交集的部分，即右上减左下为负，则交集区域面积是0可以正确表示覆盖面积）
    overlap = wh[..., 0] * wh[..., 1] # 交集面积 (B, m, n)

    union = area1[..., None] + area2[..., None, :] - overlap + eps # 计算并集区域

    if mode in ['giou']:  # giou计算方法
    ...
    eps = union.new_tensor([eps]) # 防止除以0
