...
class SODAAeval:
    ...
    def __init__(self, annotations=None, results=None, numCats=9, iouType='mAP', nproc=1):
        '''
        ...
        '''
        if not iouType:
            self._log_line('iouType not specified. use default iouType: mAP')
    ...
    def _log_line(self, line: str, level=logging.INFO):
        print_log(line.rstrip(), logger=self.logger, level=level)
    ...
    def evaluate(self):
        if mp.get_start_method(allow_none=True) != 'spawn':
            self._log_line("INFO: Multiprocessing start method is not 'spawn'. Attempting to set it...")
            try:
                mp.set_start_method('spawn', force=True)
                self._log_line("SUCCESS: Multiprocessing start method set to 'spawn'.")
            except RuntimeError as e:
                self._log_line(f"WARNING: Could not set start method to 'spawn': {e}")
                self._log_line(
        ...
        '''
        p = self.params
        self._log_line('Evaluate annotation type *{}*'.format(p.iouType))
        ...
        self._log_line(f"Found {len(devices_list)} device(s): {devices_list}. Using {self.nproc} processes.")

        self._log_line('Calculating IoUs...')
        tic = time.time()
        ...
        if self.nproc > 1:
            with Manager() as manager:
                ...
                chunksize = max(1, min(64, (num_tasks + self.nproc*8 - 1) // (self.nproc*8)))
                if chunksize == 0: chunksize = 1

                self._log_line(f"Total tasks: {num_tasks}, Processes: {self.nproc}, Chunksize: {chunksize}")
                self._log_line('Calculating IoUs...')
        ...
        toc = time.time()
        self._log_line('IoU calculation Done (t={:0.2f}s).'.format(toc - tic))

        self._log_line('Running per image evaluation...')
        tic = time.time()
        maxDet = p.maxDets[-1]
        ...
        toc = time.time()
        self._log_line('DONE (t={:0.2f}s).'.format(toc - tic))
    ...
    def accumulate(self, p=None):
        ...
        :return: None
        '''
        self._log_line('Accumulating evaluation results...')
        tic = time.time()
        if not self.evalImgs:
            self._log_line('Please run evaluate() first')
