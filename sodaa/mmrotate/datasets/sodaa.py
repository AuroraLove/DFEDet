    ...
    def evaluate(self,
        ...
        if classwise:  # Compute per-category AP
            ...
            for catId, catName in self.cat_ids.items():
                ...
                eval_results[f'{catName}'] = float(f'{ap:.3f}')
    ...
    def merge_det(self,
        ...
        for data_info, result in tqdm(zip(self.data_infos, dets), total=len(self.data_infos),
            ...
            try:
                ...
                self._log_line(f"Warning: Could not parse coordinates from filename: {filename}. Skipping.")
                continue
        ...
        self._log_line(f"Using device for NMS: {device}")
        ...
        if nproc > 1:
            try:
                ...
                self._log_line(f"Error during multiprocessing: {e}. Falling back to sequential processing.")
        ...
        stop_time = time.time()
        self._log_line('Merge results completed, it costs %.1f seconds.' % (stop_time - start_time))
...
def merge_function(info, CLASSES, iou_thr, device='cpu'):
    ...
    if all_bboxes.shape[1] == 4:
        ...
        print_log("Warning: Bounding box format is (N, 4). `nms_rotated` expects (N, 5).",logger=logger)
        pass
